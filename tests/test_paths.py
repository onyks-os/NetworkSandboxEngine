...
"""Tests for ``nse.core.paths`` - trusted resolution of the binaries TTP runs as root.
...
That is not a theoretical concern. NSE is invoked through ``sudo``, and while
...
def test_a_hostile_nft_on_path_is_not_executed(tmp_path: Path, monkeypatch) -> None:
    ...
    This is the whole point of the module. Before it, every one of NSE's
...
def test_a_world_writable_binary_is_refused(tmp_path: Path) -> None:
    ...
    with (
        patch("nse.core.paths.TRUSTED_DIRS", (str(tmp_path),)),
...
def test_a_binary_in_a_writable_directory_is_refused(tmp_path: Path) -> None:
    ...
    with (
        patch("nse.core.paths.TRUSTED_DIRS", (str(directory),)),
        pytest.raises(UnsafeBinaryError),
    ):
        resolve("nft")
...
def test_search_order_prefers_the_earlier_trusted_directory(tmp_path: Path) -> None:
    ...
    for d in (first, second):
        ...
        _plant(d, "nft")
    with (
        patch("nse.core.paths.TRUSTED_DIRS", (str(first), str(second))),
        patch("nse.core.paths._is_safely_owned", return_value=True),
    ):
...
def test_the_result_is_cached() -> None:
    ...
    first = resolve("nft")
    with patch("nse.core.paths._is_safely_owned", side_effect=AssertionError("re-checked")):
...
def test_clear_cache_forces_a_re_check() -> None:
    ...
    clear_cache()
    with (
        patch("nse.core.paths.TRUSTED_DIRS", ()),
        pytest.raises(BinaryNotFoundError),
    ):
        resolve("nft")
...
def test_the_engine_really_invokes_the_trusted_binary(tmp_path: Path, monkeypatch) -> None:
    """
    End to end: with a hostile `ip` first on $PATH, the argv the controller hands
    to subprocess still names the trusted absolute path.

    Asserting on resolve() alone would not catch a call site missed during the
    migration; this asserts on what is actually executed.
    """
    _plant(tmp_path, "ip", "#!/bin/sh\necho OWNED\n")
    monkeypatch.setenv("PATH", f"{tmp_path}:{os.environ['PATH']}")
    ...
    with (
        ...
        patch("nse.core.netns_controller._run") as run,
    ):
        NetnsController(use_nsenter=False).create_netns("nse_probe")

    argv = run.call_args_list[0][0][0]
    assert argv[0] == resolve("ip")
    assert str(tmp_path) not in argv[0]


def test_the_binaries_nse_actually_needs_are_resolvable() -> None:
    """A host that puts one of these elsewhere would make NSE fail at runtime."""
