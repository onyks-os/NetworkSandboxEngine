...
def create_app(dev_mode: bool = False) -> FastAPI:
    ...
    # Look for the compiled Svelte GUI files
    # 1. In packaged location: gui/gui_svelte/dist/
    # 2. In default build location: frontend/dist/ (fallback or custom)
    dist_path = os.path.join(os.path.dirname(__file__), "gui_svelte", "dist")
    if not os.path.isdir(dist_path):
        # Fall back to root frontend/dist
