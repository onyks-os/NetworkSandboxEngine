...
"""Trusted resolution of the external binaries NSE executes as root.

Why this module exists
----------------------

NSE runs as root - it has to, to create network namespaces and read kernel
trace events - and shells out to ``ip``, ``nft`` and ``nsenter``. Passing a bare name to :mod:`subprocess` makes the kernel resolve it
...
executed with full privileges. That is a local privilege escalation in a tool whose entire job is to be
trusted about what a firewall does.

``sudo`` usually blunts this with ``secure_path``, but "usually" is not a
security property: it is a distribution default an administrator can disable.

So NSE does not use ``$PATH``. It resolves every binary against a fixed list of
...
#: same files. ``/usr/local`` is included last because a locally built ``nft`` is
...
class BinaryNotFoundError(FileNotFoundError):
    ...
    A :class:`FileNotFoundError`, because that is exactly what it is: callers
    that already handle a missing tool keep working unchanged. TTP learned this
    the hard way - a bespoke exception type turned graceful degradation into a
    crash on every distribution that did not ship the binary.
    """


class UnsafeBinaryError(Exception):
...
def resolve(binary: str) -> str:
    ...
    for directory in TRUSTED_DIRS:
        ...
        if not _is_safely_owned(candidate):
            raise UnsafeBinaryError(
                ...
                f"non-root user could replace it before NSE runs it with full "
    ...
    raise BinaryNotFoundError(
        ...
        f"NSE deliberately does not consult $PATH, because a caller-controlled "
...
def resolve_optional(binary: str) -> str | None:
    ...
    For tools NSE can work without - ``conntrack`` when the host has no
    connection tracking to read.
